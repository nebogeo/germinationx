...
(defn world-add-object [world object]
  ...
  (if (not (world-get-object world (get object "name")))
    (do
     ...
     ; (println "added" (get object "name") (get object "tile") 
...
(defn world-summon-agent
  ...
  (merge world
         {:agents
          (map
           (fn [agent]
             (if (and (= (:name agent) agent-name)
                      (< (+ (:last-summons agent) summons-period)
                         (current-time)))
               (do
                 ;(println "sending" agent-name "to" tile-pos)
                 (merge agent {:tile tile-pos
                               :last-summons (current-time)}))
               agent))
           (world-agents world))}))
